"""Validação k-Anonimato Estrutural — Sanidade k=5, d=1 sobre ego-rede 3437 do Facebook.
...
Uso:
    python -m experiments.run_validacao_k_anonimato
    python -m experiments.run_validacao_k_anonimato --config experiments/configs/validacao_k_anonimato.yml

Saída:
    experiments/logs/validacao_k_anonimato/<seed>.jsonl  — resultado por semente
    experiments/logs/validacao_k_anonimato/summary.json  — resumo geral
...
logger = logging.getLogger("validacao_k_anonimato")
...
def evaluate_validation(results: list[dict]) -> tuple[bool, str]:
    """Evaluate the validation pass/fail criterion (DL-01).
...
def main(config_path: Path) -> int:
    """Run the k-anonymity validation; return exit code (0=pass, 1=fail)."""
    ...
    log_dir = Path(val_cfg.get("log_dir", "experiments/logs/validacao_k_anonimato/"))

    # --- Seeds ---
    seeds: list[int] = [int(s) for s in config["seeds"]]
    logger.info(
        "Running validation: k=%d, d=%d, seeds=%s, egonet=%d",
        k,
        d,
        seeds,
        egonet_id,
    ...
    # --- Summary ---
    passed, reason = evaluate_validation(results)

    summary = {
        "experiment": "validacao_k_anonimato",
        "graph": graph_info,
        "k": k,
        "d": d,
        "sigma": sigma,
    ...
    # --- Report ---
    print("\n" + "=" * 60)
    print("VALIDAÇÃO K-ANONIMATO ESTRUTURAL — RESULTADO")
    print("=" * 60)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Validação k-anonimato estrutural")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("experiments/configs/validacao_k_anonimato.yml"),
