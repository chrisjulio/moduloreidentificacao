...
# (semanas 1 a 5 + issues contingentes).
#
# Pré-requisitos:
#   - gh CLI instalado e autenticado (gh auth status)
#   - Executado na raiz do repositório, já dentro do remote correto
#
# Uso (na raiz do repo):
#   .\.github\scripts\bootstrap_issues.ps1
#
# Idempotência:
#   - Labels: --force atualiza descrição/cor se já existir.
#   - Milestones: detectados por título; se já existir, é reutilizado.
#   - Issues: NÃO são deduplicadas. Rodar duas vezes cria issues
#     duplicadas. Verificar antes de re-rodar.
#
...
Write-Host "Verificando autenticação do gh CLI..." -ForegroundColor Cyan
gh auth status

$repo = gh repo view --json nameWithOwner -q ".nameWithOwner"
Write-Host "Repositório alvo: $repo" -ForegroundColor Cyan
Write-Host ""
...
$labels = @(
    @{ name = "atomic";          color = "0E8A16"; description = "Tarefa atômica (uma sessão de Claude Code)" }
    @{ name = "component";       color = "1D76DB"; description = "Componente fechado (módulo ou arquivo)" }
    ...
    @{ name = "algorithm";       color = "B60205"; description = "Algoritmo de anonimização" }
    @{ name = "attack";          color = "D93F0B"; description = "Ataque de reidentificação" }
    @{ name = "metric";          color = "0052CC"; description = "Métrica de privacidade ou utilidade" }
    @{ name = "validation";      color = "C5DEF5"; description = "Validação empírica de propriedades" }
    @{ name = "documentation";   color = "BFD4F2"; description = "Documentação técnica ou de processo" }
    @{ name = "experiment";      color = "006B75"; description = "Configuração ou execução de experimento" }
    @{ name = "milestone-29-05"; color = "E11D21"; description = "Bloqueante do marco 29/05" }
    @{ name = "minimo";          color = "5319E7"; description = "Escopo Mínimo (obrigatório)" }
    @{ name = "desejavel";       color = "FBCA04"; description = "Escopo Desejável (contingente)" }
    @{ name = "aspiracional";    color = "C2E0C6"; description = "Escopo Aspiracional (bônus)" }
)
...
# Recupera milestones existentes para idempotência
...
# Aliases curtos para milestones
...
New-Issue -Milestone $M1 -Labels @("atomic","setup","minimo") `
  ...
  -Body @"
## Objetivo
Garantir que o ambiente local de desenvolvimento esta funcional antes de qualquer
implementacao. Esta issue NAO produz codigo de producao; produz garantia de que
o repositorio roda na maquina local.
...
- ``ruff check .`` e ``ruff format --check .`` rodam sem erros (pode haver
  warnings esperados de arquivos placeholder).
...
## Branch sugerida
``setup/local-env`` (issue fechada por PR documental, se for o caso, ou
apenas comentario de confirmacao).
"@
...
New-Issue -Milestone $M1 -Labels @("atomic","loader","minimo") `
  -Title "[S1] Loader: load_facebook_egonet(egonet_id, data_dir) -> nx.Graph" `
  -Body @"
## Objetivo
...
- Nao incluir logica de download aqui — isso e a issue do download script.
...
- Teste unitario:
  - Verifica que o grafo de saida ainda e valido (sem self-loops, sem
    multi-arestas).
  - Verifica que tamanho do grafo de saida e proximo ao original (modificacao
    "minima" no sentido pratico).
...
## Contexto
Vide [.claude/rules/anonymization.md](../../.claude/rules/anonymization.md)
...
``True``, em 3 sementes independentes, sobre ao menos uma ego-rede do
Facebook.
...
- **Criterio:** ``validate_k_anonymity`` retorna ``True`` em **100% das 3
  execucoes**.
...
New-Issue -Labels @("component","loader","desejavel") `
  ...
  -Body @"
## Objetivo
Estender o pipeline para o dataset Email-Enron (versao estatica do SNAP) como
dataset secundario.

## Condicao
So entrar em escopo se o Minimo (Facebook Ego-Nets) estiver consolidado com
