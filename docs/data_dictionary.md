...
| `reidentification_rate` (subgrafo) | `rr_subgrafo` | Taxa de reidentificação pelo ataque por subgrafo: `N_correto / N_total`, onde a associação usa isomorfismo de vizinhança 1-hop (VF2). Modelo mais próximo do adversarial de He et al. (2009). Cota teórica esperada: `≤ 1/k` **sob k-anonimato da estrutura que o ataque inspeciona** (i.e. `d ≥ 2`). **A cota não vale no regime `d=1`**, em que só o grau é anonimizado e a vizinhança 1-hop permanece distinguível (achado B1): nesse regime `rr_subgrafo > 1/k` é **esperado** para `k` alto — p.ex. Enron k=20 (`0,057 > 0,050`), ver [`docs/results_enron.md`](results_enron.md). Quanto menor, maior a resistência. |
