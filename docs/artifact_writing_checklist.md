...
| **Status** | `resolvido (declarado)` — frase canônica fixada (revisada — papel nomeado pela função no argumento; validade externa como propriedade da inferência, não rótulo do dataset): **"O Facebook Ego-Nets é o dataset principal `[M]` — grade experimental completa e inferências centrais; o Email-Enron é o dataset secundário `[D]`, de replicação cross-domain (e-mail × ego-rede de amizade), usado para verificar se as tendências qualitativas do principal se sustentam fora da topologia de ego-rede. Não é réplica simétrica nem segundo ponto de uma curva única; é essa transferência de tendências que sustenta o argumento de validade externa — propriedade da inferência, não rótulo do dataset."** Alinhada a [`scope.md`](scope.md) §3 (Facebook [M] / Enron [D]) e à DoD de #128. A cláusula "referenciar em S10-W" é aplicação editorial na redação, não pendência de verificação. |
