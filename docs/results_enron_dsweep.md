...
O padrão central do d-sweep Facebook (§5.2 de `docs/results_dsweep.md`) se replica no Enron:

- **Subgrafo enfraquece com d crescente** (em k fixo). Grupos de equivalência maiores (EGS ≈ k·d) aumentam a ambiguidade estrutural e dificultam o isomorfismo 1-hop.
- **Grau se fortalece com d crescente** (em k fixo). Anonimizar com d alto distorce mais a distribuição de graus (KS D cresce), tornando assinaturas de grau mais singulares — `reid_deg` cresce com d.

**Confirmação:** o deslocamento grau↑/subgrafo↓ com d crescente **se confirma no Enron** (D5). A tendência é robusta em k ∈ {2, 5, 10, 20} com d ∈ {2, 5, 10}.
